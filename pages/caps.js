...
const Caps = ({products}) => {
  ...
  <div className="container px-5 py-24 mx-auto">
    ...
    {Object.keys(products).length===0 && <p>Sorry all the capsItems are currently unavailable will get back soon please stay tunned🕔</p>}
...
export async function getServerSideProps(context) {
  ...
  for(let item of products){
    if(item.title in capsItem){
      if(!capsItem[item.title].color.includes(item.color) && item.availableQty>0){
        capsItem[item.title].color.push(item.color)
      }
      if(!capsItem[item.title].size.includes(item.size) && item.availableQty>0){
        capsItem[item.title].size.push(item.size)
      }
    }
    else{
      capsItem[item.title]=JSON.parse(JSON.stringify(item))
      if(item.availableQty>0){
        capsItem[item.title].color=[item.color]
        capsItem[item.title].size=[item.size]
      }
    }
  }
  return { 
    props: {products:JSON.parse(JSON.stringify(capsItem))}, // will be passed to the page component as props
  }
}
export default Caps;
