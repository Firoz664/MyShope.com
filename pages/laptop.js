    ...
    {Object.keys(products).map((item)=>{
      ...
      <div className="lg:w-1/5 md:w-1/2 p-4 w-full cursor-pointer shadow-lg m-5">
        <a className="block relative  rounded overflow-hidden">
          <img alt="ecommerce" className="m-auto h-[20vh] md:h-[26vh] block" src={products[item].img}/>
        </a>
...
export async function getServerSideProps(context) {
  ...
  for(let item of products){
    if(item.title in laptop){
      if(!laptop[item.title].color.includes(item.color) && item.availableQty>0){
        laptop[item.title].color.push(item.color)
      }
      if(!laptop[item.title].size.includes(item.size) && item.availableQty>0){
  ...
  return { 
    props: {products:JSON.parse(JSON.stringify(laptop))}, // will be passed to the page component as props
  }
}
